...
def LikelihoodRatio(
    xModel, xData, yModel, yData, tModel, theta, thetastar, sigmaY, nugget, lambda_e
):
    ...
    gp_theta = fitGP(
        xModel, xData, yModel, yData, tModel, theta, sigmaY, nugget, lambda_e
    )
    gp_thetastar = fitGP(
        xModel, xData, yModel, yData, tModel, thetastar, sigmaY, nugget, lambda_e
    )
...
def fitGP(xModel, xData, yModel, yData, tModel, tData, sigmaY, nugget, lambda_e):
    ...
    noiseVector = np.concatenate(
        (
            np.ones(np.shape(yModel)[0] * len(xModel)) * nugget,
            np.ones(np.shape(yData)[0] * len(xData)) * (sigmaY**2),
        )
    )

    # initialize gps
    gp = GaussianProcessRegressor(
        kernel=kernels.customRBF(length_scale=tData[-1], alpha=lambda_e),
        alpha=noiseVector,
    )
...
def mogpTraining(xModel, xData, yModel, yData, tModel, tData):
    ...
    tDatas = np.repeat(
        np.reshape(tData, ((1, len(tData)))), np.shape(yData)[0] * len(xData), axis=0
    )
        ...
        tModels = np.vstack(
            (
                ...
                np.hstack(
                    (
                        np.repeat(
                            np.reshape(tModel[i, :], ((1, np.shape(tModel)[1]))),
                            len(xModel),
                            axis=0,
                        ),
    ...
    xModels = np.reshape(
        np.repeat(xModel, np.shape(yModel)[0]), ((len(xModel) * np.shape(yModel)[0], 1))
    )
    xDatas = np.reshape(
        np.repeat(xData, np.shape(yData)[0]), ((len(xData) * np.shape(yData)[0], 1))
    )

    # construct training data sets
    xTrain = np.hstack((np.vstack((xModels, xDatas)), np.vstack((tModels, tDatas))))
