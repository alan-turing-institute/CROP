...
from ges.functionsV3 import derivatives, priorPPF, sat_conc
...
from ges.parameters import T_k
...
def main():
    ...
    LastDataPoint_RH = pd.DataFrame()
    LastDataPoint_T = pd.DataFrame()
    ...
    for ii in range(ndp):
        ...
        # Calculate VPD and Leaf VPD here
        SVP_air  = 610.78*np.exp((T_air-T_k)/T_air * 17.2694)
        VPD_air = SVP_air * (1-RH_air)

        SVP_v  = 610.78*np.exp((T_v-T_k)/T_v * 17.2694)
        VPD_v = SVP_v * (1-RH_air)
        ...
        DataPoint_RH = Monitored.RH_i[h2]
        DataPoint_T = Monitored.T_i[h2]

        testdp_RH = np.isnan(DataPoint_RH)
        testdp_T = np.isnan(DataPoint_T)
        # logging.info(testdp)
        # logging.info(DataPoint)

        if testdp_RH == False:
            # Divide by 100 to convert percentages to ratios
            DataPoint_RH = DataPoint_RH / 100
        else:
            # takes previous value if nan recorded
            DataPoint_RH = (LastDataPoint_RH[-1:]).DataPoint[0]

        if testdp_T == False:
            # Divide by 100 to convert percentages to ratios
            DataPoint_T = DataPoint_T + T_k
        else:
            # takes previous value if nan recorded
            DataPoint_T = (LastDataPoint_T[-1:]).DataPoint[0]

        logging.info("DataPoint:{0}".format(DataPoint))

        dpnew_RH = pd.DataFrame({"DataPoint": DataPoint_RH}, {Monitored.DateTime[h2]})
        LastDataPoint_RH = LastDataPoint_RH.append(dpnew_RH)

        dpnew_T = pd.DataFrame({"DataPoint": DataPoint_T}, {Monitored.DateTime[h2]})
        LastDataPoint_T = LastDataPoint_T.append(dpnew_T)

        SVP  = 610.78*np.exp((DataPoint_T-T_k)/DataPoint_T * 17.2694)
        VPD = SVP * (1-DataPoint_RH)

        dpnew = VPD
        DataPoint=dpnew
        LastDataPoint = LastDataPoint.append(dpnew_T)
        ...
        RHD_s = (DataPoint_RH - ym) / ystd

        ## Standardise VPD - dummy parameters for now, ym=500, ystd=50

        VPD_s = (VPD_air - 500)/50
        VPDD_s = (DataPoint - 500)/50
