function findInJson(
  ...
  // pick out rows from a list, based on measure name and/or scenario type
	...
	if (scenarioType == "ScenarioType.BAU") {
	    return obj.measure_name == measureName && obj.scenario_type == scenarioType;
	} else {
	    return obj.measure_name == measureName && obj.scenario_type == scenarioType &&
		obj.ventilation_rate == ventilationRate && obj.num_dehumidifiers == numDehumidifiers &&
...
function setScenario(ges_json, measureName, ventilationRate, numDehumidifiers, lightingShift, chart) {
    const scenarioName = ventilationRate.toString()+" ACH, "+numDehumidifiers.toString()+" DH, "+lightingShift.toString()+" hours";
   
    const sce_json = findInJson(ges_json, measureName, "ScenarioType.Test", ventilationRate, numDehumidifiers, lightingShift);
    let values_sce;
    let times_sce;
    if (sce_json != null) {
