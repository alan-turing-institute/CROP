...
def lights_energy_use(dt_from_, dt_to_):
    ...
    if not df.empty:
        ...
        energy_hour = df.groupby(by=[
                df['timestamp'].map(lambda x : pd.to_datetime("%04d-%02d-%02d-%02d" % (x.year, x.month, x.day, x.hour), format = '%Y-%m-%d-%H')), 
            ])['electricity_consumption'].sum().reset_index()
        ...
        energy_hour.loc[energy_hour['timestamp'].dt.hour < 15, 'energy_date'] = pd.to_datetime((energy_hour['timestamp'] + timedelta(days=-1)).dt.date)
        energy_hour.loc[energy_hour['timestamp'].dt.hour >= 15, 'energy_date'] = pd.to_datetime(energy_hour['timestamp'].dt.date)

        # Clasification of lights being on

        # Lights ON 1: Lights turn on at 4pm and turn off at 9am, as scheduled.
        energy_hour['lights_on_1'] = energy_hour['timestamp'].apply(lambda x: 1 if (x.hour >= 17 or x.hour < 10) else 0)
        ...
        # Lights ON 3: Lights are assumed to be on if the energy demand is over 30 kW (max load of the extraction fan)
        energy_hour['lights_on_3'] = energy_hour[col_ec].apply(lambda x: 1 if (x > 30.0) else 0)
        lights_on_cols.append('lights_on_3')

        # Lights ON 4: Lights are assumed to turn on at the time of largest energy use increase in the day, 
        #   and turn off at the time of largest energy decrease of the day.
        ...
        energy_hour.loc[np.isclose(energy_hour['dE_max'], energy_hour['dE']), 'lights_on_4'] = 1
        energy_hour.loc[np.isclose(energy_hour['dE_min'], energy_hour['dE']), 'lights_on_4'] = 0

        # repeat last?
        prev_row_value = None
        for index in energy_hour.index:
            if index > 0:
                if (np.isnan(energy_hour.loc[index,'lights_on_4']) and not np.isnan(prev_row_value)):
                    energy_hour.loc[index,'lights_on_4'] = prev_row_value
            prev_row_value = energy_hour.loc[index,'lights_on_4']

        lights_on_cols.append('lights_on_4')

        # Lights ON 5: Lights are assumed on if the energy use is over 0.9 times the days' energy use mean, 
        #   and the energy demand is over 30 kW.

        energy_hour['energy_date_mean'] = energy_hour.groupby('energy_date')[col_ec].transform('mean')
        ...
        energy_date_df = energy_hour.loc[(energy_hour['energy_date'] >= d_from) & (energy_hour['energy_date'] <= d_to)]
        energy_date_df = energy_date_df.groupby(by=['energy_date'])[lights_on_cols].sum().reset_index()
        energy_date_df['mean_lights_on'] = energy_date_df[lights_on_cols].sum(axis=1) / len(lights_on_cols)
...
def route_template(template):
    ...
    if template == "temperature":
        ...
        energy_data['data'] = '[' + lights_results_df.to_json(orient='records') + ']'
