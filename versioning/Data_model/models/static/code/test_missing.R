...
source(paste0(".","/may_live_functions_lights.R"), echo=FALSE)
...
cleanedDataPath = "./t_ee_Oct2021.RDS"
...
#set.seed(1)
#random<- randomNumbers(n = 3600,min = 744,max = 7200,col = 1)
#t_ee$EnergyCP[random] = NA

# Option 2 - regular

# Option 3 - continuous
# 7000:7048
t_ee$EnergyCP[7000:7048] = NA


reportStats(t_ee, "Missing_1")
...
getDaysPrediction = function(daysOfPredictions, forecast_timestamp) {
  #arima_results = vector("list", daysOfPredictions)
  ar = list()
  ...
  for (d in 0:daysOfPredictions){
    numDaysAgo=abs(d)
    temp_forecastDataStart = get_date_days_ago(numDaysAgo, forecast_timestamp)
    ...
    ar[[length(ar)+1]] = runModelsForSensors(temp_historicalDataStart, temp_forecastDataStart)
  }
...
daysOfPredictions = 0
result = getDaysPrediction(daysOfPredictions, forecast_timestamp-(0*SECONDS.PERDAY))

## Plot

t1m = result[[1]]$ARIMA$Temperature_FARM_16B1$records[[1]]$measure_values
t1u = result[[1]]$ARIMA$Temperature_FARM_16B1$records[[2]]$measure_values
t1l = result[[1]]$ARIMA$Temperature_FARM_16B1$records[[3]]$measure_values

tt = seq(from=forecastDataStart-(8)*SECONDS.PERDAY, to=forecastDataStart+(2)*SECONDS.PERDAY-1, by="1 hour")
t2 = (which(t_ee$FarmTime==(forecastDataStart - (8)*SECONDS.PERDAY ))):(which(t_ee$FarmTime==(forecastDataStart+ (2)*SECONDS.PERDAY))-1)
plot(tt,t_ee$Temperature_FARM_16B1[t2], type = "l", col = "blue", main = "Temperature_FARM_16B1", ylim=c(15,30))
t3 = seq(from=forecastDataStart- (0)*SECONDS.PERDAY, to=forecastDataStart+(2)*SECONDS.PERDAY-1, by="1 hour")

lines(t3,t1u,col = "red", lty = 3)
lines(t3,t1l,col = "red", lty = 3)
polygon(c(t3, rev(t3)), c(t1l, rev(t1u)), col = "mistyrose1", lty=0)

lines(t3,t1m,col = "red", lty = 2)

iftime = currentData[["tobj_list"]]$Temperature_FARM_16B1$FarmTime[1:7192]
ifdata = currentData[["tobj_list"]]$Temperature_FARM_16B1$Sensor_temp[1:7192]
lines(iftime,ifdata,col = "green", lty = 3)
...
#ttrain = seq(from=forecastDataStart-(1)*SECONDS.PERDAY, to=forecastDataStart-(0)*SECONDS.PERDAY-1, by="1 hour")
#t2train = (which(t_ee$FarmTime==(forecastDataStart - (1)*SECONDS.PERDAY ))):(which(t_ee$FarmTime==(forecastDataStart- (0)*SECONDS.PERDAY))-1)
...
t1u = result[[1]]$ARIMA$Temperature_Farm_16B2$records[[2]]$measure_values
t1l = result[[1]]$ARIMA$Temperature_Farm_16B2$records[[3]]$measure_values
...
iftime = currentData[["tobj_list"]]$Temperature_Farm_16B2$FarmTime[1:7192]
ifdata = currentData[["tobj_list"]]$Temperature_Farm_16B2$Sensor_temp[1:7192]
lines(iftime,ifdata,col = "green", lty = 3)
lines(tt,t_ee$Temperature_Farm_16B2[t2], col = "blue")
#test2 = data.frame(t3,test,tt,t_ee$Temperature_Farm_16B2[t2])

#
t1m = result[[1]]$ARIMA$Temperature_Farm_16B4$records[[1]]$measure_values
t1u = result[[1]]$ARIMA$Temperature_Farm_16B4$records[[2]]$measure_values
t1l = result[[1]]$ARIMA$Temperature_Farm_16B4$records[[3]]$measure_values
...
iftime = currentData[["tobj_list"]]$Temperature_Farm_16B4$FarmTime[1:7192]
ifdata = currentData[["tobj_list"]]$Temperature_Farm_16B4$Sensor_temp[1:7192]
lines(iftime,ifdata,col = "green", lty = 3)
lines(tt,t_ee$Temperature_Farm_16B4[t2], col = "blue")
#test2 = data.frame(t3,test,tt,t_ee$Temperature_Farm_16B4[t2])

ifdata = currentData[["tobj_list"]]$Temperature_Farm_16B4$EnergyCP[6950:7192]
iftime = currentData[["tobj_list"]]$Temperature_Farm_16B4$FarmTime[6950:7192]
plot(iftime,ifdata, type = "l", col = "blue")
