...
getForecastTimestamp = function(latestTimeStamp = ? Date) {
  ...
  tenDaysIntoPast = 10*SECONDS.PERDAY
  #list_f_timestamps = seq(from=latestTimeStamp-fourDaysIntoPast, to= latestTimeStamp-twoDaysIntoPast, by="2 days")
  list_f_timestamps = seq(from=latestTimeStamp-tenDaysIntoPast, to= latestTimeStamp, by="1 days")
...
splitTrainingTestData = function (tobj, historicalDataStart, forecastDataStart) {
  ...
  trainsel = 1:(which(tsel$FarmTime==(forecastDataStart)))
  # indices for forecasting
  testsel = rep((which(tsel$FarmTime==(forecastDataStart))-23):(which(tsel$FarmTime==(forecastDataStart))),2)
...
setupModels = function(split.Data, sensorID, time_forecast) {
  ...
  runbstsPipeline = function(split.Data, sensorID){
    ...
    records.mean.bsts = list(measure_id = MEASURE_ID$Humidity_Mean, measure_values = results.bsts$mean)
    records.median.bsts = list(measure_id = MEASURE_ID$Humidity_Median, measure_values = results.bsts$median)
...
reportStats = function(a_t_ee, label="Source") {
  ...
  if (label == "Today"){
    stats.Humidity.numNA.a_t_ee = sprintf("%s Temperature NumNA/Rows: %i/%i = %f\n", 
                                           label,
                                           sum(is.na(a_t_ee$Humidity_FARM_16B1)),
                                           length(a_t_ee$Humidity_FARM_16B1),
                                           sum(is.na(a_t_ee$Humidity_FARM_16B1))/length(a_t_ee$Humidity_FARM_16B1))
    cat(stats.Humidity.numNA.a_t_ee)
  }
...
forecast_timestamp = currentData$forecast_timestamp - 1*SECONDS.PERDAY

daysOfHistoryForTraining = 100
