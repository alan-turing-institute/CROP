...
def model(t,z, climate, ACHvec, iasvec, daynum, count, h1, h2, ndhvec, lshiftvec):
    ...
    QP_i_m = A_m_exp*0.4*dsat*H_fg/(rho_i*c_i)*(Sh/Le)*(lam/d_m)*(C_w - sat_conc(T_m))
...
def derivatives(h1, h2, numDays, paramsinput, ndp, filePathWeather=None):
    ...
    for i in range(NP):
        ...
        params = [climate, ACH, ias, daynum, count, h1, h2, ndh, lshift]
