...
ax1.fill_between(
    t[:-73], T_air[3][:-73] - 273.15, T_air[4][:-73] - 273.15, color="red", alpha=0.2
)
...
ax3.fill_between(
    t[:-73], 100 * RH_air[3][:-73], 100 * RH_air[4][:-73], color="red", alpha=0.2
)
...
y1 = {
    "T<Tmin": np.sum(testTBAU_low),
    "T OK": 73 - np.sum(testTBAU) - np.sum(testTBAU_low),
    "T>Tset": np.sum(testTBAU),
}
...
y2 = {
    "T<Tmin": np.sum(testTSE_low),
    "T OK": 73 - np.sum(testTSE) - np.sum(testTSE_low),
    "T>Tset": np.sum(testTSE),
}
...
y3 = {
    "RH<RHmin": np.sum(testRHBAU_low),
    "RH OK": 73 - np.sum(testRHBAU) - np.sum(testRHBAU_low),
    "RH>RHset": np.sum(testRHBAU),
}
...
y4 = {
    "RH<RHmin": np.sum(testRHSE_low),
    "RH OK": 73 - np.sum(testRHSE) - np.sum(testRHSE_low),
    "RH>RHset": np.sum(testRHSE),
}
